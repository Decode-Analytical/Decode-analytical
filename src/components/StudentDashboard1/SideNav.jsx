...
const SideNav = ({ toggleOpenMenu, toggleCloseMenu, menuOpen }) => {
  ...
  return (
    <div
      ...
      ${menuOpen ? "w-[250px]" : "w-[85px]"} 
         h-screen text-gray3 fixed left-0 top-0 bg-red-950 px-4 gap-10 z-50 transition-[width] duration-700 ease-in-out uppercase bg-shadow`}
    >
      <span
        ...
        >
          <Link to="/admin-dashboard/profile">
            <p className="text-xl min-w-[160px] font-medium">
              {/* {userProfile?.firstName} */}Sekinat
            </p>
          </Link>
          <p className="font-semibold px-2 bg-[#005508] text-white rounded-md text-sm text-white1 w-fit uppercase">
            {"instructor"}
          </p>
        </div>
      </div>
